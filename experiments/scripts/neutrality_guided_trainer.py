...
class NeutralityGuidedTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        This method computes the standard seq2seq loss and optionally adds
        a neutrality guidance component based on classifier scoring of generated text.
    ...
    def _compute_neutrality_loss(self, model, inputs) -> torch.Tensor:
        ...
        This method:
        1. Generates sequences using the current model (greedy for speed)
        2. Decodes the generated token IDs to text
        3. Scores the text with the neutrality classifier
        4. Computes a loss based on falling short of the target neutrality rate

        Args:
            model: The model to generate with
            inputs: Input batch dictionary containing input_ids

        Returns:
            torch.Tensor: Scalar loss value for neutrality guidance
        """
        ...
        # If mean neutrality >= target, loss = 0
        # If mean neutrality < target, loss = (target - mean)
